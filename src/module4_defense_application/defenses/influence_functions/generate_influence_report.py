...
def generate_report(json_file, md_file):
    with open(json_file, "r") as f:
        data = json.load(f)

    lines = []
    lines.append("# Defense Report — Influence Functions\n")

    lines.append("## Overview\n")
    lines.append(f"- **Defense:** {data['defense']}")
    lines.append(f"- **Attack Targeted:** {data['attack']}")
    lines.append(f"- **Influence Method:** {data['method']}")
    ...
    if "example_removed" in data and data["example_removed"]:
        ...
        lines.append("The following examples illustrate removed samples identified as influential outliers.\n")
        lines.append("Each image is named using the format:\n\n")
        lines.append("```")
    ...
    print(f"[✔] Influence Functions report generated at: {md_file}")
