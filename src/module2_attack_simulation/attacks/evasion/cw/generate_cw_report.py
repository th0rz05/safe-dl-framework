...
def generate_cw_report(json_file, md_file):
    with open(json_file, "r") as f:
        data = json.load(f)

    lines = []
    lines.append("# Evasion Attack Report — Carlini & Wagner (C&W)\n")
    lines.append("## Overview\n")
    lines.append(f"- **Attack Type:** {data['attack_type']}")
    lines.append(f"- **Targeted Attack:** {data['targeted']}")
    lines.append(f"- **Confidence Parameter:** {data['confidence']}")
    lines.append(f"- **Initial Constant:** {data['initial_const']}")
    lines.append(f"- **Learning Rate:** {data['learning_rate']}")
    lines.append(f"- **Max Iterations:** {data['max_iterations']}")
    lines.append(f"- **Binary Search Steps:** {data['binary_search_steps']}\n")
    ...
    lines.append(
        "The following examples illustrate adversarial inputs generated by the C&W attack. "
    ...
    print(f"[✔] C&W report generated at: {md_file}")
