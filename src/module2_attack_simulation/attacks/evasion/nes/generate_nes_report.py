...
def generate_nes_report(json_file, md_file):
    with open(json_file, "r") as f:
        data = json.load(f)

    lines = []
    lines.append("# Evasion Attack Report — NES\n")
    lines.append("## Overview\n")
    lines.append(f"- **Attack Type:** {data['attack_type']}")
    lines.append(f"- **Epsilon:** {data['epsilon']}")
    lines.append(f"- **Sigma (noise std deviation):** {data['sigma']}")
    lines.append(f"- **Learning Rate:** {data['learning_rate']}")
    lines.append(f"- **Number of Queries:** {data['num_queries']}")
    lines.append(f"- **Batch Size (noise samples per step):** {data['batch_size']}\n")
    ...
    lines.append(
        "The following examples illustrate adversarial inputs generated by the NES attack. "
    ...
    print(f"[✔] NES report generated at: {md_file}")
