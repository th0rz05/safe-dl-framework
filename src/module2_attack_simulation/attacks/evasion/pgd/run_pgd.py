...
def run_pgd(testset, profile, class_names):
    ...
    out_dir = os.path.join("results", "evasion", "pgd")
    ...
    for idx, (x, y) in enumerate(tqdm(testloader, desc="PGD Attack")):
        ...
        adv_x = pgd_attack(trained_model, x, y, epsilon, alpha, num_iter, random_start)
        ...
        if len(example_log) < 5:
            ...
            fname = f"pgd_{idx}_{class_names[y.item()]}_{class_names[pred_adv.item()]}.png"
    ...
    print(f"[✔] PGD Attack complete. Clean accuracy: {clean_acc:.4f}, Adversarial accuracy: {adv_acc:.4f}")
    ...
    out_dir = "results/evasion/pgd"
    os.makedirs(out_dir, exist_ok=True)

    with open(os.path.join(out_dir, "pgd_metrics.json"), "w") as f:
        json.dump(result, f, indent=2)

    print("[✔] PGD attack complete. Metrics saved to pgd_metrics.json")

    generate_pgd_report(
        json_file="results/evasion/pgd/pgd_metrics.json",
        md_file="results/evasion/pgd/pgd_report.md"
    )

    print("[✔] PGD report generated in results/evasion/pgd/pgd_report.md")
