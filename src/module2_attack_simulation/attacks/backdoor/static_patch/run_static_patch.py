...
def run_static_patch(trainset, testset, valset, model, profile, class_names):
    ...
    for idx in poison_indices:
        ...
        #visualize the patch of the first 3 poisoned samples
        if count < 3:
            plt.imshow(patched_img.permute(1, 2, 0).numpy())
            plt.axis('off')
            plt.title(f"Poisoned Image with Patch at {patch_position}")
            plt.show()
    ...
    # === Poison test set (apply patch to ALL samples) ===
    ...
    for idx in tqdm(range(len(patched_testset)), desc="Patching test set"):
        ...
        update_poisoned_sample(patched_testset, idx, image, target_class)

    # === Evaluate on patched test set ===
    print("[*] Evaluating on patched test set...")
    patched_acc, patched_per_class = evaluate_model(model, patched_testset, class_names=class_names)
    ...
    result = {
        ...
        "accuracy_patched_testset": patched_acc,
        "per_class_clean": clean_per_class,
        "per_class_patched": patched_per_class,
