...
-   `baseline_accuracy.json`  
    Located in: `module2_attack_simulation/results/`  
    Contains the clean accuracy before any attacks were applied.
    
-   `{attack_category}/{attack_name}_metrics.json`  
    Located in: `module2_attack_simulation/results/`  
    Contains the evaluation results of the model under attack (no defenses).
    
-   `{attack_category}/{attack_name}/{defense_name}_results.json`  
    Located in: `module4_defense_application/results/`  
    Contains evaluation results of the model after the defense was applied.
    
-   `profile.yaml`  
    Located in: `profiles/`  
    Contains the selected dataset, model, and list of attacks/defenses to be evaluated.
...
1. **Profile Selection**  
   The user selects a threat profile that defines which attacks and defenses were applied.

2. **Data Collection**  
   For each attack, the corresponding metrics and applied defenses are loaded from disk.

3. **Defense Evaluation**  
   A dynamic evaluation function (e.g., `evaluate_data_cleaning`) is called for each defense. All evaluation functions follow a unified signature to allow automated mapping without conditional statements.

4. **Scoring Computation**  
   For each evaluated defense, the mitigation score, CAD score, cost score, and final score are computed using the formulas defined in Section 3.

5. **Report Generation**  
   The final scores are saved to `defense_evaluation.json`. Optionally, the report script (`generate_defense_evaluation_report.py`) can be executed to generate a full Markdown summary.

### 4.4 Modular Design

All scoring functions are defined in:

- `defense_score_utils.py`: Core scoring formulas (mitigation, CAD, final score).
- `defense_utils.py`: One `evaluate_<defense>` function per defense, handling data parsing and calling the scoring functions.

This modularity allows easy addition of new defenses without modifying the main evaluation script.
